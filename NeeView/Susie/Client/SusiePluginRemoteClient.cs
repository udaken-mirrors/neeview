...
namespace NeeView.Susie.Client
{
    public class SusiePluginRemoteClient : IDisposable
    {
        ...
        public void Connect()
        {
            ...
            var subProcessFileName = Path.Combine(Environment.LibrariesPath, "Libraries\\NeeView.Susie.Server.exe");

            // 開発中は直接プロジェクトを参照する
            if (Environment.IsDevPackage)
            {
                subProcessFileName = Path.GetFullPath(Path.Combine(Environment.AssemblyFolder, @"..\..\..\..\..\NeeView.Susie.Server\bin\x86", Environment.ConfigType, @"net7.0-windows\NeeView.Susie.Server.exe"));
            }
